...
export default function TestPage() {
  ...
  const toProperCase = (string) => { return string.charAt(0).toUpperCase() + string.substring(1).toLowerCase(); };
  // let profile;
  // let items = {};
        ...
        <Dropdown>
          ...
          <Dropdown.Menu onAction={(key) => setLabel(key)}>
            ...
            {tagSelectOptions.map((v) => {
              return <Dropdown.Item key={v.value}><p className="pText">{v.label}</p></Dropdown.Item>; // proper/title case
            })}
          </Dropdown.Menu>
        </Dropdown>
        <Spacer x={1} />
