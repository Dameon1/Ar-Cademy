 ...
 */
export function generateTransactionChunksAsync(file) {
  //return async (source: AsyncIterable<Buffer>): Promise<NonNullable<Transaction['chunks']>> => {
    ...
    let leaves,root,proofs;

    async function createdChunk(){
     leaves = await generateLeaves(chunks);
     root = await buildLayers(leaves);
